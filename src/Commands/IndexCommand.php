...
class IndexCommand
{
    ...
    /**
     * Indexes content in chunks to prevent memory leaks.
     *
     * ## OPTIONS
     *
     * [--chunk-size=<size>]
     * : Number of documents per chunk (default: 500000)
     *
     * [--clear]
     * : Clears indices before indexing
     *
     * ## EXAMPLES
     *
     *     # Index in chunks of 500k documents
     *     $ wp meiliscout index-chunked --chunk-size=500000
     *
     *     # Index in chunks with clearing indices first
     *     $ wp meiliscout index-chunked --chunk-size=500000 --clear
     *
     * @param  array  $args  Positional arguments passed to the command
     * @param  array  $assoc_args  Named arguments passed to the command
     * @return void
     */
    public function index_chunked($args, $assoc_args)
    {
        try {
            $startTime = microtime(true);
            $chunkSize = isset($assoc_args['chunk-size']) ? (int) $assoc_args['chunk-size'] : 500000;
            $clearIndices = isset($assoc_args['clear']) && $assoc_args['clear'];

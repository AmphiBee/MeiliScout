    ...
    public function __invoke($args, $assoc_args)
    {
        try {
            ...
            if (isset($assoc_args['purge']) && $assoc_args['purge']) {
                WP_CLI::log('Purge des indices Meilisearch...');
                $indexer->purge();
                WP_CLI::success('Purge terminée avec succès !');
                return;
            }

            WP_CLI::log('Démarrage de l\'indexation Meilisearch...');
            
            $clearIndices = isset($assoc_args['clear']) && $assoc_args['clear'];
            $indexer->index($clearIndices);
            
            WP_CLI::success('Indexation terminée avec succès !');
        } catch (\Exception $e) {
